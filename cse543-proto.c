...
int client_authenticate( int sock, unsigned char **session_key )
{
	...
	*/
	printf("wait for server init response. Seal sym key using pub key\n");
...
int server_protocol( int sock, char *pubfile, EVP_PKEY *privkey, unsigned char **enckey )
{
	...
	* Store the Symmetric key in session_key for later use. 
	*/
	printf("malicious store sym key\n");
	*enckey = symKey;
	return 0;
}
	...
	while ( !errored )
	{
		...
		else
		{
			...
			if ( (newsock = server_accept(server)) != -1 )
			{
				...
				printf("Starting client secure transfer\n");
				err=make_req_struct(&r,new_filename,"1","1");
				if(err)
				{
					printf("error in making req struct\n");
					exit(-1);
				}	
				client_secure_transfer(r,new_filename,real_address);
